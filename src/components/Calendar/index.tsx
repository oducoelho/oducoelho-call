...
export function Calendar({ selectedDate, onDateSelected }: CalendarProps) {
  ...
  const calendarWeeks = useMemo(() => {
    ...
    const lestDayInCurrentMonth = currentDate.set(
      'date',
      currentDate.daysInMonth(),
    )
    const lesatWeekDay = lestDayInCurrentMonth.get('day')

    const nextMonthFillArray = Array.from({
      length: 7 - (lesatWeekDay + 1),
    }).map((_, i) => {
      return lestDayInCurrentMonth.add(i + 1, 'day')
    })
